...
| `GraphEdge` | Directed edge: mutable source/target node + port IDs (`setSourceEndpoint()`/`setTargetEndpoint()`, for interactive endpoint drag), `EdgeAttributes` (visual), mutable `edgeType` (named relationship kind, mirroring but unlike `GraphNode.getNodeType()` — reassignable, since a preset can be applied to an existing edge), `Map<String,Object>` properties reserved for optional domain-level data (currently `label`/`sourceLabel`/`targetLabel` text) |
...
| `CanvasOverlayPanel` | Transparent `JPanel` at `OVERLAY_LAYER`; state machine (`IDLE / EDGE_CREATION / EDGE_DRAGGING`); renders selection handles (single selection) or highlight outlines (multi-selection), the rubber-band marquee rectangle, port anchor circles, and the edge-drag rubber-band line; transparent to mouse events in `IDLE` state except near resize handles or (also IDLE-only) a selected edge's endpoint grip handles. Edge-endpoint dragging (`isDraggingEndpoint`/`draggingSourceEnd`/`endpointDragCurrent`) is an independent boolean track parallel to `isResizing`, not part of the `State` enum — both are selection-driven gestures orthogonal to the toolbar-mode-driven `State` |
